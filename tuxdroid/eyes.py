...
class Eyes():
    ...
    def __init__(self, config: dict):
        # Get logger
        self.logger = logging.getLogger("tuxdroid").getChild("eyes")
    ...
    def calibrate(self):
        ...
        GPIO.add_event_detect(self.moving_sensor, GPIO.RISING,
                              callback=self.wings_rotation_callback,
                              bouncetime=int(BOUNCE_TIME * 1000))

    def wings_rotation_callback(self, channel):
        ...
        if not self.is_calibrated:
            return 
        print("MOVING DETECTION. Current pos:", self.position)
